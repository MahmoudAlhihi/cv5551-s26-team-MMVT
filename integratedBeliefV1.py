...
if __name__ == "__main__":
    ...
    def safe_gohome(arm, display):
        ...
        if code == 0 and pos:
            arm.set_position(pos[0], pos[1], pos[2] + 40,
    ...
    try:
        ...
        while not found_target:
            ...
            # Observe: take median red-pixel count over 10 frames
            display.set_label(f"step {step}: observing")
            pump_sleep(display, 0.3)
            red_counts = []
            for _ in range(10):
                display.pump()
                cvimg, _ = capture.latest()
                rp, _ = red_mask_count(cvimg)
                red_counts.append(rp)
                time.sleep(0.03)
            red_pixels = int(np.median(red_counts))

            found = red_pixels > RED_PIXEL_THRESHOLD
            print(f"  Red pixels (median of 10): {red_pixels} → "
