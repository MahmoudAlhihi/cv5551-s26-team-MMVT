...
from checkpoint6 import  _sam3_processor
...
def build_full_mask(image):
    ...
    with torch.no_grad(), torch.autocast('cuda', dtype=torch.bfloat16):
        ...
        saved_state = dict(state)  # save the encoded state

        mask_dict = {}
        for color, prompt in PROMPT_MAP.items():
            # Reset prompts but keep the encoded image
    ...
    if not masks:
        print("SAM3 failed for all colors")
        return None, None

    full_mask = masks[0].copy()
    for m in masks[1:]:
...
def detect_all_cubes(image: numpy.ndarray, point_cloud: numpy.ndarray, t_cam_robot: numpy.ndarray) -> list[dict]:
    ...
    """
    full_mask, mask_dict = build_full_mask(image)
    ...
    for i in range(1, num_labels): # label 0 is the background
        ...
        # instead of dominant_colour()
        best_color = None
        best_overlap = 0

        for color, m in mask_dict.items():
            if m is None:
                continue
            overlap = numpy.sum((m > 0) & (comp_mask > 0))
            if overlap > best_overlap:
                best_overlap = overlap
                best_color = color

        colour = best_color if best_color else 'unknown'
    ...
    try:
        ...
        for rank, cube in enumerate(cubes):
            ...
            if pt[2] > 0: # only project if point is in front of the cam
                ...
                cv2.putText(cv_image, label, (u, v - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.55,
