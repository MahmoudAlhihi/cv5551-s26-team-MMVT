...
if __name__ == "__main__":
    ...
    from sweep import sweep_table, ROBOT_IP, GRIPPER_LENGTH, X_MIN, X_MAX, Y_MIN, Y_MAX
    ...
    try:
        ...
        # Build per-sample (x, y, magnitude) arrays from both sweep passes.
        # X sweep: arm travels X_MAX → X_MIN at fixed Y_MIN.
        # Y sweep: arm travels Y_MIN → Y_MAX at fixed X_MIN.
        # Each sample index maps linearly to the robot-frame coordinate.
        n_x = len(data_x)
        x_coords = np.linspace(X_MAX, X_MIN, n_x) / 1000.0   # mm → m
        y_coords_x = np.full(n_x, Y_MIN / 1000.0)             # fixed Y during X sweep

        n_y = len(data_y)
        y_coords = np.linspace(Y_MIN, Y_MAX, n_y) / 1000.0    # mm → m
        x_coords_y = np.full(n_y, X_MIN / 1000.0)             # fixed X during Y sweep

        xs   = np.concatenate([x_coords,   x_coords_y])
        ys   = np.concatenate([y_coords_x, y_coords])
        mags = np.concatenate([np.asarray(data_x, dtype=float),
