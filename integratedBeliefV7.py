...
def visualise_belief_simple(bf: BeliefFilter, title: str = "Belief",
    ...
    print(f"\n[Visualise] {title}")
    print(f"  {'region':>6}  {'P(target)':>10}  {'mean':>8}  {'state':>10}")
    ...
    for r in bf.regions:
        ...
        mean = bf.region_score(r)
        ruled = r.region_id in bf._ruled_out
        state = "RULED OUT" if ruled else "active"
        print(f"  {r.region_id:>6}  {prob*100:>9.2f}%  {mean*100:>8.3f}%  {state:>10}")

        if ruled:
            marker_colour = [0.3, 0.05, 0.05]
        elif prob >= 0.99:
            marker_colour = [0.0, 0.85, 0.0]  # green when target found
        else:
            marker_colour = list(_belief_to_rgb(min(1.0, prob)))
        ...
        label_text = f"R{r.region_id}: {prob*100:.1f}% (μ {mean*100:.2f}%)"
        anchor = np.array([
            r.centre_xyz[0],
            r.centre_xyz[1],
