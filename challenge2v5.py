...
from checkpoint1challenge2 import grasp_cube_large, grasp_cube_small, place_cube, GRIPPER_LENGTH, TRANSIT_SPEED, TRANSIT_ACCEL
import numpy 
...
>>>>>>> 8650b2540f103a9793e5d3068d29c54fd3edfc19
...
def stack_cubes(arm, cubes: list[dict]) -> None:
    ...
    cubes[0] is the immovable base (never moved).  All subsequent cubes are
    picked and placed on top of the growing tower.

    Collision-safe transit
    ----------------------
    Cubes vary in size, so a naive straight-line move from one cube's pick
    height to another's place height can clip a neighbouring cube that is
    taller than the current transit altitude.

    Solution: compute SAFE_TRANSIT_Z_MM once as the absolute Z (in mm,
    robot frame) of the highest cube-top in the scene plus TRANSIT_CLEARANCE_MM
    of headroom.  Every lateral move is broken into three explicit legs:
      1. Vertical-only ascent  to safe_transit_z  (directly above current pos)
      2. Lateral-only transit  at safe_transit_z  (to directly above target)
      3. Descent is handled inside place_cube, which descends from wherever
         the arm already is to PRE_GRASP_HEIGHT above the stack target — since
         the arm is already directly above the stack at safe_transit_z this
         descent is purely vertical and collision-free.

    The same safe-height ascent is done after placing before the next pick.
    """
    if not cubes:
    ...
    # ── Safe transit height ──────────────────────────────────────────────────
    # Highest absolute top-face Z across all cubes (robot frame, metres)
    # cube centre Z + half-height = absolute top face Z
    TRANSIT_CLEARANCE_MM = 30  # extra headroom above tallest cube top face

    highest_top_m = max(
        c['t_robot'][2, 3] - c['size_m'] / 2.0   # top face Z (note: robot Z
        for c in cubes                              # increases upward here)
    )
    # Convert to the sign convention used by arm.set_position:
    # the arm API receives -z (negated robot-frame Z) for the Z argument,
    # so safe_transit_z_mm is the value we pass as the 'z' argument to
    # set_position — it needs to be MORE NEGATIVE than any cube top.
    # Robot frame: higher object = larger positive z_robot.
    # arm.set_position z argument = -z_robot * 1000  (see grasp functions).
    # So a larger z_robot means a more-negative API z argument.
    # We want to be ABOVE all cubes, i.e. z_robot > highest_top → API z < -(highest_top*1000)
    safe_transit_z_api = -(highest_top_m * 1000) - TRANSIT_CLEARANCE_MM
    print(f"Safe transit z (API): {safe_transit_z_api:.1f} mm  "
          f"(highest cube top: {highest_top_m*1000:.1f} mm robot-frame)")

    # Base cube reference
    base_t    = cubes[0]['t_robot'].copy()
    base_x_mm = base_t[0, 3] * 1000
    base_y_mm = base_t[1, 3] * 1000

    # Extract base yaw for all place moves
    rot_base       = Rotation.from_matrix(base_t[:3, :3])
    base_r, base_p, base_yaw = rot_base.as_euler('xyz', degrees=False)

    # z_top: current top face of the growing stack (robot-frame metres, positive up)
    ...
    for i in range(1, n):
        cube   = cubes[i]
        pick_t = cube['t_robot']
        pick_x = pick_t[0, 3] * 1000
        pick_y = pick_t[1, 3] * 1000

        rot_pick           = Rotation.from_matrix(pick_t[:3, :3])
        pick_r, pick_p, pick_yaw = rot_pick.as_euler('xyz', degrees=False)

        print(f"\n── Cube {i+1}/{n}: {cube['colour']}  "
              f"({cube['size_m']*1000:.1f} mm) ──")

        # ── 1. Grasp (leaves arm at PRE_GRASP above pick cube) ───────────────
        if cube['size_m'] > 0.02:
            grasp_cube_large(arm, pick_t, cube['size_m'])
        else:
            grasp_cube_small(arm, pick_t, cube['size_m'])

        # ── 2. Lift straight up to safe transit height ───────────────────────
        # Stay at pick XY, just change Z — purely vertical, nothing to clip
        arm.set_position(pick_x, pick_y, safe_transit_z_api,
                         pick_r, pick_p, pick_yaw,
                         is_radian=True, wait=True,
                         speed=TRANSIT_SPEED, mvacc=TRANSIT_ACCEL)

        # ── 3. Lateral transit to directly above the stack ──────────────────
        # Z stays at safe_transit_z, only XY changes — clears all cubes
        arm.set_position(base_x_mm, base_y_mm, safe_transit_z_api,
                         base_r, base_p, base_yaw,
                         is_radian=True, wait=True,
                         speed=TRANSIT_SPEED, mvacc=TRANSIT_ACCEL)

        # ── 4. Place (descends vertically from safe height onto stack) ───────
        ...
        # place_cube leaves arm at PRE_GRASP_HEIGHT above stack_target

        # ── 5. Lift straight up to safe transit height after placing ─────────
        # Needed so the next pick transit doesn't clip the growing stack
        arm.set_position(base_x_mm, base_y_mm, safe_transit_z_api,
                         base_r, base_p, base_yaw,
                         is_radian=True, wait=True,
                         speed=TRANSIT_SPEED, mvacc=TRANSIT_ACCEL)
